...
- 概览「项目推进」新增 **＋ 新建项目**，创建后自动生成标准工程目录并进入全局项目列表。
- 灵感、研究日志、笔记、里程碑、工作总结、文献的项目字段默认保持空白，改为 **项目标签 + ＋ 选择器**；只能从已有项目中勾选，支持一个条目关联多个项目。待办也改为已有项目下拉选择，不再自由输入项目名。
- Agent API Key 改为 **环境变量读取**。设置页只保存环境变量名称（默认 `OPENAI_API_KEY`），不会把 Key 写入 `app.json` 或其它工作台密钥文件。
- Agent 默认提供三套请求模式：`默认（不附加参数）`、`Qwen · 低思考`、`Qwen · 无思考`，仍可在设置页 JSON 中自行修改。
- Agent 发送消息采用乐观渲染：按 Enter 后用户消息会立即进入对话区，输入框立即清空；模型请求期间显示思考动画与耗时。若接口返回 `reasoning` / `reasoning_content` 等字段，可按设置决定是否显示可折叠的模型思考过程。
- 知识图谱点击画布空白处即可取消节点高亮，恢复完整图谱显示。
