(() => {
  ...
  const state = {
    ...
    editorMode: 'split', milestoneView: 'timeline', graphView: '2d', graph: null, graphSelected: null, previewSeq: 0,
  ...
  function setHeader(route){
    const [ey,title] = PAGE_META[route] || ['WORKBENCH', route];
    $('#page-eyebrow').textContent=ey; $('#page-title').textContent=title;
    const d=new Date(); $('#page-date').textContent = d.toLocaleDateString('zh-CN',{year:'numeric',month:'long',day:'numeric',weekday:'short'});
    document.title = `${title} · ${state.config?.app?.app_name || '科研工作台'}`;
  }
  ...
  function searchResultRow(r,i){const label=r.source==='chat'?'Agent 对话':r.source==='todo'?'待办':(KIND_LABEL[r.kind]||r.kind);return `<button class="global-search-row" type="button" data-search-result="${i}"><span class="global-search-kind">${esc(label)}</span><span class="global-search-copy"><strong>${esc(r.title||'未命名')}</strong><small>${esc(r.project||'')}${r.project?' · ':''}${esc(r.excerpt||'')}</small></span><span>↗</span></button>`}
  ...
  function wireDocFilters(kind){ const run=debounce(async()=>{const q=$('#doc-search').value,status=$('#doc-status').value,project=$('#doc-project').value;const url=`/api/docs?kind=${kind}&q=${encodeURIComponent(q)}&status=${encodeURIComponent(status)}&project=${encodeURIComponent(project)}`;state.docs=await api(url);$('#doc-list').innerHTML=docItems(state.docs);wireDocList(kind);},180); $('#doc-search').oninput=run;$('#doc-status').onchange=run;$('#doc-project').onchange=run; }
  async function createAndSelect(kind){ const doc=await api('/api/docs',{method:'POST',body:{kind,title:`未命名${KIND_LABEL[kind]}`}}); await renderDocsPage(kind); setTimeout(()=>selectDoc(doc.id),10); }
  function showEmptyEditor(kind){ $('#doc-editor').className='card doc-editor empty-editor'; $('#doc-editor').innerHTML=`<div class="empty"><div><div class="empty-symbol">${kind.toUpperCase()}</div>选择一条${KIND_LABEL[kind]}，或点击左侧 ＋ 新建</div></div>`; }
  ...
  function renderDocEditor(doc){
    ...
    $('#doc-editor').innerHTML=`<div class="form-grid">
      ...
      <div class="field span-2"><label>项目</label><div class="project-picker-row"><div class="project-chip-box" id="f-projects" data-projects="${esc(JSON.stringify(doc.projects||[]))}"></div><button type="button" class="secondary-btn project-add-btn" id="f-project-add" title="从已有项目中添加">＋</button></div><span class="field-help">默认为空；点击 ＋ 从已有项目中勾选，可同时关联多个项目。</span></div>
      ...
      <div class="field ${special?'span-4':'span-4'}"><label>标签（逗号分隔）</label><input id="f-tags" value="${esc((doc.tags||[]).join(', '))}" placeholder="标签1, 标签2, 标签3"></div>
    </div>
  ...
  function animateMilestones3D(){
    ...
    viewport.onpointerdown=e=>{if(e.target.closest('.timeline-3d-controls'))return;drag={x:e.clientX,y:e.clientY,mode:(e.altKey||e.button===2)?'rotate':'pan'};moved=false;viewport.setPointerCapture?.(e.pointerId);viewport.classList.add('dragging')};
    ...
    viewport.onpointerup=e=>{drag=null;viewport.classList.remove('dragging');try{viewport.releasePointerCapture?.(e.pointerId)}catch{}};
    ...
    $$('[data-ms-id]',stage).forEach(x=>x.onclick=()=>{if(moved){moved=false;return}cancelAnimationFrame(raf);state.milestoneView='docs';renderMilestoneView();setTimeout(()=>selectDoc(x.dataset.msId),10)});
  }
  ...
  async function renderGraphPage(){ const g=await api('/api/graph');state.graph=g;
    ...
    $('#main').innerHTML=`<div class="card card-pad"><div class="card-head"><div><div class="card-kicker">RELATION MAP</div><h3>知识图谱</h3></div><div class="view-tabs"><button data-gview="2d" class="${state.graphView==='2d'?'active':''}">2D</button><button data-gview="3d" class="${state.graphView==='3d'?'active':''}">3D 星图</button></div></div>
      <div class="graph-wrap"><div class="graph-canvas-wrap"><canvas class="graph-canvas" id="graph-canvas"></canvas></div><aside class="card graph-side"><h3 id="graph-node-title">选择节点</h3><div class="graph-node-info" id="graph-node-info">点击任意节点后，将高亮相邻节点与关系边。导出严格使用当前可见节点和关系。</div><div style="margin-top:12px"><button class="primary-btn" id="graph-bundle" disabled>整理关联 Markdown</button></div><div class="graph-filter"><strong>显示节点类别</strong><div class="graph-filter-grid">${filterKinds.map(([k,label])=>`<label><input type="checkbox" data-graph-kind="${k}" ${state.graphKinds.has(k)?'checked':''}> ${label}</label>`).join('')}</div><div class="graph-filter-actions"><button class="ghost-btn" id="graph-filter-all">全选</button><button class="ghost-btn" id="graph-filter-docs">仅知识条目</button></div></div><div class="graph-filter"><strong>关系来源</strong><div class="graph-filter-grid">${relationKinds.map(([k,label])=>`<label><input type="checkbox" data-graph-relation="${k}" ${state.graphRelations.has(k)?'checked':''}> ${label}</label>`).join('')}</div><div class="row-meta">隐藏“标签”或“项目”节点时，对应关系边也会立即从图谱和导出结果中移除。</div></div></aside></div>
    </div>`;
    ...
    $('#graph-bundle').onclick=()=>state.graphSelected&&openGraphBundleFromNode(state.graphSelected);renderGraphCanvas();
  }
  ...
  function renderGraphCanvas(){ $$('[data-gview]').forEach(b=>b.classList.toggle('active',b.dataset.gview===state.graphView)); const c=$('#graph-canvas');if(!c)return;const g=filteredGraph();if(state.graphSelected&&!g.nodes.some(n=>n.id===state.graphSelected.id)){state.graphSelected=null;$('#graph-node-title').textContent='选择节点';$('#graph-node-info').textContent='当前筛选已隐藏原选中节点。';$('#graph-bundle').disabled=true} drawGraph(c,g,{mode:state.graphView,interactive:true,selectedId:state.graphSelected?.id,onSelect:onGraphSelect}); }
  function nodeProjects(n){return (n.projects||[]).length?n.projects:(n.project?[n.project]:[])}
  function onGraphSelect(n){state.graphSelected=n||null;if(!n){$('#graph-node-title').textContent='选择节点';$('#graph-node-info').textContent='点击任意节点后，将高亮相邻节点与关系边；点击画布空白处可取消高亮。导出严格使用当前可见节点和关系。';$('#graph-bundle').disabled=true;renderGraphCanvas();return}$('#graph-node-title').textContent=n.label;const projects=nodeProjects(n);const tags=n.tags||[];$('#graph-node-info').innerHTML=`类型：${esc(KIND_LABEL[n.kind]||({project:'项目',tag:'标签'}[n.kind])||n.kind)}<br>项目：${esc(projects.join(', ')||'—')}<br>标签：${esc(tags.join(', ')||'—')}<br>状态：${esc(n.status||'—')}<br><span class="row-meta">相邻节点和边已高亮；点击空白处恢复全图。导出会继承当前图谱筛选。</span>`;$('#graph-bundle').disabled=false;renderGraphCanvas();}
  function drawGraph(canvas,g,opts={}){
    ...
    let rotX=.25,rotY=0,drag=false,dragMoved=false,last=[0,0],selected=byId.get(opts.selectedId)||null; const R=Math.min(w,h)*(opts.mini?.28:.34);
    ...
    function project(n){ if(opts.mode==='3d'){const cy=Math.cos(rotY),sy=Math.sin(rotY),cx=Math.cos(rotX),sx=Math.sin(rotX);let x=n.bx*cy-n.bz*sy,z=n.bx*sy+n.bz*cy,y=n.by*cx-z*sx;z=n.by*sx+z*cx;const ss=560/(560+z);return{x:w/2+x*ss,y:h/2+y*ss,z,s:ss}} return{x:w/2+n.bx,y:h/2+n.by,z:0,s:1}; }
    function paint(){if(canvas._graphRenderToken!==token)return;ctx.clearRect(0,0,w,h);const pos=new Map(nodes.map(n=>[n.id,project(n)]));const neighborIds=new Set();if(selected){for(const e of edges){if(e.source===selected.id)neighborIds.add(e.target);else if(e.target===selected.id)neighborIds.add(e.source)}}
      ...
      const sorted=[...nodes].sort((a,b)=>project(a).z-project(b).z);for(const n of sorted){const p=pos.get(n.id),isSel=selected?.id===n.id,isNei=neighborIds.has(n.id),base=n.virtual?8:6,rad=(base+(isSel?4:isNei?2:0))*p.s;ctx.globalAlpha=selected?(isSel||isNei?1:.32):1;ctx.beginPath();ctx.arc(p.x,p.y,Math.max(3,rad),0,Math.PI*2);ctx.fillStyle=n.kind==='project'?getCss('--accent'):kindColor(n.kind);ctx.fill();if(isSel){ctx.strokeStyle=getCss('--text');ctx.lineWidth=3;ctx.stroke()}else if(isNei){ctx.strokeStyle=getCss('--accent');ctx.lineWidth=1.8;ctx.stroke()}if(!opts.mini||nodes.length<18||isSel||isNei){ctx.font=`${isSel||isNei?'700':'400'} ${Math.max(9,(isSel?12:isNei?11:10)*p.s)}px Microsoft YaHei`;ctx.fillStyle=isSel||isNei?getCss('--text'):getCss('--muted');ctx.fillText(truncate(n.label,isSel?22:14),p.x+rad+4,p.y+3)}}ctx.globalAlpha=1;canvas._positions=pos; }
    function hit(x,y){let best=null,bd=20;for(const n of nodes){const p=canvas._positions?.get(n.id);if(!p)continue;const d=Math.hypot(x-p.x,y-p.y);if(d<bd){bd=d;best=n}}return best}
    canvas.onpointerdown=e=>{drag=true;dragMoved=false;last=[e.offsetX,e.offsetY];canvas.setPointerCapture?.(e.pointerId)};canvas.onpointermove=e=>{if(!drag||opts.mode!=='3d')return;const dx=e.offsetX-last[0],dy=e.offsetY-last[1];if(Math.hypot(dx,dy)>1)dragMoved=true;rotY+=dx*.008;rotX+=dy*.006;last=[e.offsetX,e.offsetY];paint()};canvas.onpointerup=e=>{const n=hit(e.offsetX,e.offsetY);drag=false;if(!dragMoved){selected=n||null;paint();opts.onSelect?.(n||null)}};canvas.onpointercancel=()=>{drag=false};paint();
    if(opts.mode==='3d'&&state.config?.app?.ui?.animations&&!matchMedia('(prefers-reduced-motion: reduce)').matches){const loop=()=>{if(canvas._graphRenderToken!==token||!document.body.contains(canvas))return;if(!drag){rotY+=.0015;paint()}requestAnimationFrame(loop)};requestAnimationFrame(loop)}
  }
  ...
  function renderSettingsTab(tab,cfg){const p=$('#settings-panel'),app=cfg.app,rss=cfg.rss;if(tab==='general'){const mig=app.workspace_migration||{};p.innerHTML=`<div class="card-head"><div><div class="card-kicker">GENERAL</div><h3>基础设置</h3></div></div><div class="form-grid"><div class="field span-2"><label>工作台名称</label><input id="set-app-name" value="${esc(app.app_name)}"></div><div class="field span-2"><label>副标题</label><input id="set-subtitle" value="${esc(app.subtitle)}"></div><div class="field span-2"><label>Workspace</label><input value="${esc(app.workspace)}" readonly><span class="field-help">独立工程固定使用项目目录下的 Workspace；如需迁移工程，整体复制项目即可。</span></div><div class="field span-2"><label>旧数据自动识别 / 非破坏迁移</label><select id="set-migrate"><option value="1" ${mig.enabled!==false?'selected':''}>启用</option><option value="0" ${mig.enabled===false?'selected':''}>关闭</option></select></div></div><div style="margin-top:14px"><button class="primary-btn" id="save-settings">保存</button></div>`;$('#save-settings').onclick=async()=>{app.app_name=$('#set-app-name').value;app.subtitle=$('#set-subtitle').value;app.workspace_migration={...mig,enabled:$('#set-migrate').value==='1',copy_legacy_data:true};await api('/api/config/app',{method:'POST',body:app});toast('基础设置已保存');await loadBootstrap();};}
