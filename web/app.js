(() => {
  ...
  function onGraphSelect(n){state.graphSelected=n||null;if(!n){$('#graph-node-title').textContent='选择节点';$('#graph-node-info').textContent='点击任意节点后，将高亮相邻节点与关系边；点击画布空白处可取消高亮。导出严格使用当前可见节点和关系。';$('#graph-bundle').disabled=true;renderGraphCanvas();return}$('#graph-node-title').textContent=n.label;const projects=nodeProjects(n);const tags=n.tags||[];$('#graph-node-info').innerHTML=`类型：${esc(kindLabel(n.kind))}<br>项目：${esc(projects.join(', ')||'—')}<br>标签：${esc(tags.join(', ')||'—')}<br>状态：${esc(n.status||'—')}<br><span class="row-meta">相邻节点和边已高亮；点击空白处恢复全图。导出会继承当前图谱筛选。</span>`;$('#graph-bundle').disabled=false;renderGraphCanvas();}
  ...
  function kindColor(kind){const m={idea:'#2a9d8f',journal:'#5b8def',note:'#6f7a8a',milestone:'#c6863b',summary:'#845ec2',literature:'#2673a7',tag:'#b56a9d',project:'#087f8c'};return m[kind]||getCss('--accent')}
